...
function Get-FileChecksumSha256 { param([string]$Path)
...
function Get-PrimaryKeyColumns { param([string]$Database,[string]$Table)
    $q = @"
...
function Get-NonPrimaryColumns { param([string]$Database,[string]$Table,[string[]]$Pk)
...
function New-MigrationScriptFromDiff {
    param([string]$TargetDatabase,[string]$ShadowDatabase,[string[]]$Tables,[string]$OutputDir)
    ...
    $summary = Get-ChangesSummary -TargetDatabase $TargetDatabase -ShadowDatabase $ShadowDatabase -Tables $Config.TrackedTables
        ...
        if ($diffs.Count -eq 0) { Write-Host 'No changes detected.' } else {
            ...
            switch ($Action) {
                ...
                'Migrate' {
                    New-MigrationScriptFromDiff -TargetDatabase $TargetDatabase -ShadowDatabase $ShadowDatabase -Tables $diffs.Table -OutputDir $OutputMigrationDir
                }
                'Cancel' {
